...
class SigninAPIView(APIView):
    ...
    def post(self, request):
        ...
        if user is not None and user.is_active:
            login(request, user)
            response = dict(cp_request_change_password=user.request_change_password)
    ...
    def post(self, request):
        ...
        if user and user.is_authenticated:
            ...
            if form.is_valid():
                ...
                if user is not None and user.is_active:
                    login(request, user)
                    response = dict(data=dict(user_id=user.pk, cp_request_change_password=user.request_change_password),
                                    message=u'Новый пароль успешно установлен')
                    response = Response(response)
                    response.set_cookie('user_id', value=user.id, httponly=False)
                    return response
                else:
