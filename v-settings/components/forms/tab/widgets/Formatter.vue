...
export default {
    ...
    watch: {
        text: function() {
            if (typeof this.text !== "undefined") this.value = this.text;
        },
    ...
    mounted() {
        const callMe = () => {
            if (this.loadStatus.mainData) {
                const el = document.getElementById(
                    "formatter-" + this.options.codename
                );
                new Formatter(el, {
                    toolbar: [
                        [
        ...
        // Поставил таймаут, потому что иногда не успевает подгрузиться value
        setTimeout(() => {
            callMe();
        }, 200);
    },
