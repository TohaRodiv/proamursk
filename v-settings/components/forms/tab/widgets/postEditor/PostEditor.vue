<template>
    <div class="post-editor-wrapper">
        <div class="post-editor-empty-wrapper" v-if="!content.length">
            <div class="tab-left-label col4"></div>
            <div
        ...
        <div v-if="content.length" class="post-editor-outer-wrapper">
            ...
            <div
                ...
                <div class="post-editor-add-section-helper-top" v-if="secDex !== 0">
                    <div style="margin-top: 32px;" v-if="section.marginTop && (sectionHovered !== secDex || sectionOnDrag)">{{section.marginTop + ' em'}}</div>
                    <div
                ...
                <div class="tab-left-label col4 post-editor-block-info">
                    <div>{{numberToWord(section.columns.length)}} {{multiEnds(section.columns.length)}} / {{concatColumnNumbers(section)}}</div>
                    <div class="post-editor-block-info-buttons" v-if="sectionHovered === secDex && !sectionOnDrag && !widgetOnDrag">
                        <button class="button blue-button-small icon-moving" @mousedown="startSectionDnd(secDex, $event)"></button>
                        <button v-if="sectionIsConfigurable" class="button blue-button-small icon-edit" @click="showChangeSectionPopup(secDex, section)"></button>
                        <button class="button blue-button-small icon-copy" @click="copySection(section)"></button>
                        <button class="button delete-button-round-small icon-close" @click="deleteSection(secDex)"></button>
                    </div>
                </div>
                <div class="post-editor-inned-block-wrapper">
                    <div
                        ...
                        <div :style="setSectionPadding(section, block) + (`box-shadow: ${isOnDrag(section) ? '0 2px 8px 0 rgba(35, 41, 46, 0.15)' : ''}`)">
                            <div style="height: 15px; width: 100%;" v-if="!block.widgets || !block.widgets.length"></div>
                            <div
                            ...
                            <div
                                ...
                                <div>{{section.paddingBottom + ' em'}}</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="post-editor-add-section-helper-bottom" v-if="secDex !== content.length - 1">
                    <div style="margin-top: 7px;" v-if="section.marginBottom && (sectionHovered !== secDex || sectionOnDrag)">{{section.marginBottom + ' em'}}</div>
                    <div
                            v-if="sectionHovered === secDex && !sectionOnDrag && !widgetOnDrag"
                            class="post-editor-add-section-wrapper">
        ...
        >
            <div class="tab-left-label col4"></div>
            <div class="post-editor-error-message">{{ options.message }}</div>
        </div>
    </div>
</template>
