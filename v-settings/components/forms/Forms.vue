            ...
            <div class="forms-container-inner">
                <div class="forms-sidebar-container" v-if="formsCurrentMode === 'moving'">
                    <div class="forms-sidebar-header">
                        ...
                        <div class="forms-sidebar-header-bold-text" v-if="isNaN(+(this.$route.params.id))">
                            0. Новый
                        </div>
                        <div class="forms-sidebar-header-bold-text" v-if="!isNaN(+(this.$route.params.id))">
                        ...
                        <div v-for="(status, index) in getMovingData.status_log" :key="index">
                            <div class="forms-sidebar-node-main">
                                <div class="forms-sidebar-round-passed" id="11" :class="{'forms-sidebar-round-success': status.status === 'complete', 'forms-sidebar-round-canceled': status.status === 'canceled'}">
                                    <span class="forms-sidebar-number" :class="{'forms-sidebar-number-success': status.status === 'complete' || status.status === 'canceled'}">
                                        {{index}}
                                    </span>
                                </div>
                                <div class="forms-sidebar-body-bold-text" :class="{'forms-sidebar-body-bold-text-success': status.status === 'complete', 'forms-sidebar-body-bold-text-canceled': status.status === 'canceled'}">
                                    {{status.status_name}}
                                </div>
                                <div class="forms-sidebar-body-info" :class="{'forms-sidebar-body-info-success': status.status === 'complete', 'forms-sidebar-body-info-canceled': status.status === 'canceled'}">
                ...
                <div class="forms-wrapper" :style="computedFormsWrapperWidth">
                        ...
                        <div class="tabs-wrapper">
                            <div class="forms-tabs-container" v-if=" (config) ? config.length > 1 && formsCurrentMode !== 'moving' : false">
                                ...
                                >
                                    ...
                                    <div v-if="tab.id === currentTabId" class="forms-tabs-item-selected-border"/>
                                </div>
                            </div>
                    ...
                    <formSidebar
                        ...
                        :actions="compInitActions"
                        :item="compUnionData"
                    />
                </div>
            </div>

            <deletePopup @closeDeletePopup="showDeletePopup = false" v-if="showDeletePopup" @onDelete="typeOfMove === 'onDelete'"/>
            <cancelPopup @closePopup="showCancelPopup = false" v-if="showCancelPopup" @cancelMoving="cancelMoving"/>
            <infoPopup @closeInfoPopup="showInfoPopup = false" @infoPopupResume="saveEntity('moveForward')" v-if="showInfoPopup"/>
            <unsavedPopup @closePopup="showUnsavedPopup = false" @continueConfirmed="continueConfirmed = true" :pathForRouter="unsavedPopupDaWae" v-if="showUnsavedPopup"/>
            <historyPopup @closePopup="showHistoryPopup = false" v-if="showHistoryPopup"/>

            <!-- <div v-if="showAdditionalPreloaderData"> -->
            <div class="forms-buttons-container" v-if="computedIsEditForm">
                <div class="forms-buttons-container-inner">
                    ...
                    <button v-if="compHasEditPermission" class="button borderless-button forms-save-and-add-button" @click="saveEntity('save&push')">
                        Сохранить и добавить
                    </button>
                    <button v-if="compHasEditPermission" class="button forms-save-button" @click="saveEntity('save')">
                        Сохранить
                    </button>
                </div>
            </div>
            <div class="forms-buttons-container" v-if="computedIsAddForm">
                <div class="forms-buttons-container-inner">
                    <button class="button borderless-button forms-save-and-add-button" v-if="$route.params.id !== 'form' && compHasAddPermission" @click="saveEntity('add&push')">
                        Сохранить и добавить
                    </button>
                    <button v-if="compHasAddPermission" class="button forms-save-button" @click="($route.params.id === 'form') ? saveEntity('save') : saveEntity('add')">
                        Сохранить
                    </button>
                </div>
            </div>
